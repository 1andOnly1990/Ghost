...
object Prompts {
    const val SYSTEM_PROMPT = """You are the Game Master of "Legion", a text-based cyberpunk narrative RPG set in Veridia City.
...
On first session (no chat history): Begin in media res. Devon has just confirmed a successful offshore data transfer in a rain-soaked service alley. Aura is giving him the all-clear. Then the Chronos Dynamics 'Nomad' G-4 explodes at the end of the alley. Begin there. Do not explain the mechanics. Just start the story."""
    ...
    const val WORLD_SIMULATION_PROMPT = """You are the world simulation engine for "Legion", an ongoing cyberpunk narrative.
  ...
  "newOpportunities": ["new_node_id"]
    ...
    fun buildPrompt(
        ...
        val historyContext = if (chatHistory.isNotEmpty()) {
            val historyLines = chatHistory.takeLast(6).joinToString("\n") { msg ->
                val speaker = if (msg.isPlayerMessage) "PLAYER" else msg.entity.name
                "[$speaker]: ${msg.content}"
            }
            "\n=== RECENT CONVERSATION ===\n$historyLines"
        } else ""
